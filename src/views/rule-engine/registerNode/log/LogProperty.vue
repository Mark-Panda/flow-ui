<template>
  <div>
    <el-form ref="propertyFormRef" :model="propertyForm" label-position="top" :rules="rules">
      ...
      <el-form-item label="日志模板" prop="template">
        <el-input v-model="propertyForm.template" type="textarea" :rows="3" placeholder="支持${msg.data}格式的变量"></el-input>
      </el-form-item>
    </el-form>
    <div class="mt15">
...
const rules = {
  ...
  template: [
    { required: true, message: '请输入日志模板', trigger: 'blur' },
  ],
};
...
onMounted(() => {
  ...
  propertyForm.template = props.nodeData.properties.template || '${msg.data}';
});
</script>
